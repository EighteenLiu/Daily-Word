...
def build() -> None:
    ...
    add_para(document, "该小区垃圾分类情况好的方面主要有：1.四类垃圾桶设置齐全，且容器完好、洁净；2.有开展垃圾分类宣传工作；3.站点指导员按时上岗；4.小区垃圾投放规范，无垃圾乱堆乱放、投放不规范等现象；5.厨余、可回收物和有害垃圾桶分类纯净；6.有装修垃圾和大件垃圾投放点并且规范。存在的问题是：{{ community.overall_problem_summary }}")
    add_jinja(document, "{%p else %}")
    add_para(document, "存在的问题是：{{ community.overall_problem_summary }}")
    ...
    add_para(document, "（2）小区公示牌：{% if community.notice_board_text != \"无问题\" %}{{ community.notice_board_text }}{% endif %}")
    ...
    add_para(document, "预约收集，集中密闭运输。")
    add_jinja(document, "{%p endif %}")
    add_jinja(document, "{%p if community.community_litter_text %}")
    add_para(document, "（5）小区垃圾乱堆乱放、投放不规范现象：{{ community.community_litter_text }}")
    add_image_loop(document, "community.community_litter_images")
    add_jinja(document, "{%p endif %}")
